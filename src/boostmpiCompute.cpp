...
int main (int argc, char **argv) {
    ...
    if (world.rank()==root_process){
        ...
        size_t row = setN.size();
        size_t col = setN[0].size();

        vector1D normMaxs(col, -999999);
        vector1D normMins(col, 9999999);

        //find maximums and minimums of each column
        for (int i = 0; i < row; ++i) {
            for (int j = 0; j < col; ++j) {
                if(setN[i][j]>= normMaxs[j])
                    normMaxs[j]=setN[i][j];
                if(setN[i][j]<=normMins[j])
                    normMins[j]=setN[i][j];
            }
        }


        setNNorm = normalize(setN, normMaxs, normMins);

        //read in setQ
        setQ = loadDataFromLinuxSystem(inputPathSetQ, Q, D);

        setQNorm = normalize(setQ, normMaxs, normMins);
    ...
    if(world.rank()==root_process){
        ...
        for (int i = 0; i < gatheredCandidateSet.size(); ++i) {
            for (int j = 0; j < gatheredCandidateSet[i].size(); ++j) {
                out << gatheredCandidateSet[i][j] << ',';
            }
            out<<'\n';
        }
        out.close();
